...
def integral(h,xmin,xmax,ymin,ymax):
	axis=h.GetYaxis()
        axisX=h.GetXaxis()
	bmin=axis.FindBin(ymin)
	bminX=axisX.FindBin(xmin)
	bmax=axis.FindBin(ymax)
	bmaxX=axisX.FindBin(xmax)
	integral=h.Integral(bminX,bmaxX,bmin,bmax)
...
for part in particle:
	for filename in tag:
                ...
                print(part+" "+filename+" "+str(integral(histo[count],6040,6100,25,50))+" "+str(histo[count].GetMaximum())+"\n")
		count=count+1
"""
		if count==0:
